...
export default function VideoCard({ video, isFavorite = false, onToggleFavorite, index = 0 }: VideoCardProps) {
  return (
    <div className="group relative flex-shrink-0 w-[140px] sm:w-[170px] md:w-[220px] lg:w-[240px] transition-all duration-300"
      ...
      <Link href={`/watch/${video.id}`} className="block relative aspect-[2/3] rounded-lg overflow-hidden bg-zinc-900 shadow-lg">
        {video.thumbnail_url ? (
        ...
        <div className="absolute inset-x-0 bottom-0 p-3 opacity-0 group-hover:opacity-100 group-active:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0">
          <div className="flex items-center gap-1.5 mb-2">
            ...
            <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); onToggleFavorite?.(video.id); }}
              className="w-8 h-8 flex items-center justify-center rounded-full border border-zinc-400/50 bg-zinc-900/60 hover:bg-zinc-700 text-white transition-all hover:scale-110 backdrop-blur-sm cursor-pointer">
