      ...
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Email" required
            className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-3 text-white placeholder:text-zinc-500 outline-none focus:border-red-500/50 transition-colors" />
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Contrasena (min 6 caracteres)" required minLength={6}
            className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-3 text-white placeholder:text-zinc-500 outline-none focus:border-red-500/50 transition-colors" />
        ...
        <p className="text-zinc-500 text-sm text-center mt-6">
          Ya tienes cuenta? <Link href="/login" className="text-red-400 hover:text-red-300">Inicia sesion</Link>
        </p>
      </div>
    </div>
  );
